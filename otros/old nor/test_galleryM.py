...
from gallery.gallery_selection import GalleryChapters
...
class VentanaPrincipal(QMainWindow):
    ...
    def _configVentanaPrincipal(self):
        ...
        self.__addwidgetGalleryChapters(central_widget)
        self.wg = GalleryBase(central_widget)
        # self.test_wg()
        self.test_btn()
        self.btn.clicked.connect(self.setImages)
        self.splitter.addWidget(self.gc)
        ...
        self.wg.cellClicked.connect(self.selectCellPs)
    ...
    def setImages(self):
        path = 'T:/TAG/EJECUTABLES/RECURSOS/pro_plex/poses'
        sf = SearchFiles(path)
        images = sf.getImages()
        self.wg.AR = 6/4
        self.wg.setImages(images=images, cols=3)

    def __addwidgetGalleryChapters(self, parent):
        self.gc = GalleryChapters(parent)
        self.gc.setDim(rows=5, cols=1)
        self.gc.widthColumnsEquals()
        self.gc.setHeightRow(80)

    def selectCellPs(self, row=None, col=None):
        wg = self.wg.cellWidget(row, col)
        if wg:
            name = wg.STEM
            path = wg.PATH
            print('name :: ', name)
            self.gc.addImage(path)
